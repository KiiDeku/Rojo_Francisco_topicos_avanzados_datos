...
DECLARE
	...
	FROM Clientes t2
	JOIN Pedidos t1 ON t2.ClienteID = t1.ClienteID
	GROUP BY t2.ClienteID, t2.Nombre
...
BEGIN
	...
	LOOP
		FETCH pedidost_cursor INTO v_cliente_id, v_cliente_nombre, v_total_pedidos;
		EXIT WHEN pedidost_cursor%NOTFOUND;
		DBMS_OUTPUT.PUT_LINE('Cliente: ' || v_cliente_nombre || ' | Total de Pedidos: ' || v_total_pedidos);
	END LOOP;
